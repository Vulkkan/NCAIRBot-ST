...
def reply(prompt, context) -> str:
    ...
    # highest-voted response
    return max_value


def botResponse(prompt):
    response = reply(prompt, context)
    return response
...
context = [text[i:j] for i, j in zip([0] + split_indices, split_indices + [None])]
